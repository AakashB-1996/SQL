...
with sub as (
    select Wimbledon as gs from Championships 
    union all
    select Fr_open as gs from Championships 
    union all
    select US_open as gs from Championships 
    union all
    select Au_open as gs from Championships 
)
select p.player_id,p.player_name,count(*) as grand_slams_count
from sub s join Players p on s.gs = p.player_id
group by 1,2
